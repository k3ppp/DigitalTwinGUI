...
class EnvScene(Scene):
    ...
    def __init__(self, window, name):
        ...
        self.planes = [0]*30 #pre allocated walls, can we append??
        return
    ...
    def buildPlaneXY(self, x, y, z, dx, dy, vis):
        ...
        planexy = Model(vertices=vertices_xy)    
    ...
    def buildPlaneXZ(self, x, y, z, dx, dz, vis):
        ...
        planexy = Model(vertices=vertices_xz)    
        plane = self.modelRenderer.addModel(planexy, createTransformationMatrix(0, 0, 0, 0, 0, 0))
        return plane
    ...
    def buildPlaneYZ(self, x, y, z, dy, dz, vis):
        ...
        planexy = Model(vertices=vertices_yz)    
        plane = self.modelRenderer.addModel(planexy, createTransformationMatrix(0, 0, 0, 0, 0, 0))
        return plane

    def __createRoom(self):
        self.planes[0] = self.buildPlaneXY(0, 0, 0, 15, 15, 0)
        self.modelRenderer.setColor(self.planes[0], (1, 1, 0, 1))
        self.planes[1] = self.buildPlaneXZ(0, 0, 0, 15, 15, 0)
        self.modelRenderer.setColor(self.planes[1], (1, 0, 1, 1))
        self.planes[2] = self.buildPlaneYZ(0, 0, 0, 15, 15, 0)
        self.modelRenderer.setColor(self.planes[2], (0, 1, 1, 1))
